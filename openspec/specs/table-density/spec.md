...
- [density-variant-type](../density-variant-type/spec.md) - One shared density vocabulary (`cozy | compact | crammed`) for design-toolkit collection components, exported once from the package so Tree, List, Menu, Accordion, and Table describe density with the same type instead of four drifting copies.
