...
test.describe('Button Memory Leak Tests', () => {
  test('should not leak memory when clicking Default button 100 times', async ({
    page,
  }) => {
    // Persist tests snapshots to isolated dir
    const snapshotDir = join(DEFAULT_CONFIG.snapshotDir, 'button1');
    ...
    // Click the Default button 100 times
    for (let i = 1; i <= 100; i++) {
