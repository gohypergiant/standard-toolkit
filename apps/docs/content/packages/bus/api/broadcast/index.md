...
- `Events` - Union type of all event payloads. Should be a union of `Payload<type, data>` types. Defaults to `Payload<string, StructuredCloneable>`.
...
```typescript
static getInstance<Type extends BasicPayload>(
...
```typescript
setEventsEmitOptions(
  events: Map<Events['type'], EmitOptions | null>
): void
```

Sets emit options for multiple event types at once.
...
> **Good to know:** Due to the event-driven nature, `ping()` is not synchronous. Listen for ping/echo events and check the `connected` property to maintain an accurate list of connections.
...
- [useBus](../react/index.md#usebus) - React hook wrapper for Broadcast
