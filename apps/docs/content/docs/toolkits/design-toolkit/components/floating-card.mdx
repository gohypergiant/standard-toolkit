---
title: FloatingCard
description: Renders children into a resizable, draggable floating panel using React portals
...
Title displayed in the floating card header. When omitted the header shows no title and the card is announced as "Floating card".
...
Initial x and y coordinates for the floating card's top-left corner. If not specified, the card appears at a default position.
...
1. Creates a floating panel if `isOpen` is `true`
