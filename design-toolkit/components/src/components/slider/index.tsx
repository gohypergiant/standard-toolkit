...
export interface SliderProps
  extends Omit<RACSliderProps, 'value' | 'defaultValue'> {
  className?: string;
  showInput?: boolean;
  showOutput?: boolean;
...
export const Slider = ({
  ...
  return (
    <RACSlider
      ...
      {...props}
    >
      <Label className={labelVariants({ orientation, layout })}>{label}</Label>
      {showInput && (
...
function SliderInput({ className }: { className?: string }) {
  //@ts-ignore react type mismatch between @types/react 18 and 19
