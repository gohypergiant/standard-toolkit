...
export function checkSharedImports(imports) {
  ...
  const hasSharedControls = imports.some(
    (imp) =>
      (imp.source.includes('shared-controls') ||
        imp.source.includes('utils/controls')) &&
      imp.specifiers.some(
  ...
  if (!hasSharedControls) {
    issues.push({
      ...
      severity: 'error',
      message:
        'Missing import for createStandardParameters from controls utilities',
      fix: "Add: import { createStandardParameters } from '^storybook/utils/controls';",
    });
  }
...
export function checkMetaStructure(meta, _filepath) {
  ...
  if (meta.parameters && !hasCreateStandardParametersUsage(meta.parameters)) {
    issues.push({
      ...
      severity: 'warning',
      message: 'Parameters not using createStandardParameters',
      fix: 'Use createStandardParameters for consistent parameter structure',
    });
  }
