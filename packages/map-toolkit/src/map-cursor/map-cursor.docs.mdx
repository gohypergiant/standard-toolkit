...
1. **Mode Owner Priority**: If a mode owner exists (from MapModeStore), only that owner can change the cursor
2. **Most Recent Wins**: In default/ownerless mode, the most recent cursor request is displayed

This prevents conflicts when multiple features want to control the cursor while respecting the map mode ownership hierarchy.
...
Component A requests cursor 'pointer' (owner: 'feature-a')
  → Mode owner check
  → If mode is owned by someone else: REJECT
  → If mode is default or owned by requester: ACCEPT
  → Store cursor and make it current
...
When a non-owner tries to change the cursor in an owned mode, a rejection event is emitted with a detailed reason. This can be used to display notices or toast messages to users.
...
- ✅ Mode owner can change cursor
- ❌ Non-owners cannot change cursor (rejected with event)
