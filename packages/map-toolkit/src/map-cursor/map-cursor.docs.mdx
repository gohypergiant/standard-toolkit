...
Component A requests cursor 'pointer' (owner: 'feature-a')
  → Check if cursor is different from currently stored cursor for this owner
  → If same cursor: Skip (no event, no storage update)
  → If different: Store cursor request (even if will be rejected)
  → Mode owner check
  → If mode is owned by someone else:
    → REJECT immediate application
    → Emit rejection event
    → Cursor stored for later (auto-applies when requester becomes owner)
  → If mode is default or owned by requester:
    → Check if cursor actually changes from current display
    → If same: Skip notification (already displaying this cursor)
    → If different: Accept and notify
    → Update map canvas cursor
```
...
- ✅ Mode owner can change cursor immediately
- ⚠️ Non-owners' requests are rejected for immediate application but **stored**
- 🔄 Stored cursor auto-applies when requester becomes mode owner

**No mode owner (default mode):**
- ✅ Any component can change cursor
- ✅ Most recent request wins
- Each owner can have one cursor set at a time

**Duplicate Prevention:**
- 🚫 Same cursor request from same owner → No event, no update
- 🚫 Cursor already displayed → No change event
- ✅ Only actual state changes trigger events and notifications
...
```tsx
// ✅ Good - Using useMapCursorEffect (auto-cleanup)
function MyFeature() {
...
// ❌ Bad - No cleanup (cursor persists after unmount)
function MyFeature() {
  const { requestCursorChange } = useMapCursor(MAIN_MAP_ID);
...
1. **Empty parameters** - `cursor` or `requestOwner` is empty/whitespace → throws error
2. **Mode ownership** - Another component owns the mode → cursor change rejected
