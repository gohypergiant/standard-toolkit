...
```typescript
{
  ...
  longitude: number;
  zoom: number;
  pitch: 0;              // Always 0 in 3D
  rotation: number;      // Always 0 in 3D
  projection: 'globe';
  view: '3D';
}
```
...
```tsx
// Switch to 2.5D view (sets pitch to 60°)
bus.emit(CameraEventTypes.setView, {
  id: instanceId,
  view: '2.5D',
});
...
Set the rotation angle (bearing/heading). Only works in 2D and 2.5D modes. A right-drag / Ctrl + left-drag on a `BaseMap` emits this event too (see [Mouse Camera Control](#mouse-camera-control)).
...
Set the pitch (tilt) angle. Only works in 2.5D mode with Mercator projection. A right-drag / Ctrl + left-drag on a `BaseMap` emits this event too (see [Mouse Camera Control](#mouse-camera-control)).
...
On a `BaseMap`, the mouse drives rotation and pitch directly through the camera store (the same `setRotation`/`setPitch` events you can emit programmatically):

- **Right-drag** (or **Ctrl + left-drag**): rotate and pitch the camera.
- **Left-drag**: pan, as usual.
- **Tilting in a flat 2D view** auto-promotes the view to 2.5D so pitch can apply. The tilt continues from the current pitch (0° in 2D), growing smoothly with the drag — it does not jump to the 60° default that an explicit `setView('2.5D')` would apply.

Each drag continues from the camera's current angle rather than resetting, and pitch is clamped to the 0-85° range. See the BaseMap docs for wiring details.
...
### Pitch and Rotation Constraints

Pitch applies in 2.5D mode. Rotation applies in 2D and 2.5D modes. Both are ignored in 3D mode. A mouse tilt gesture (see [Mouse Camera Control](#mouse-camera-control)) auto-promotes a 2D view to 2.5D before applying pitch, so you don't have to switch views first:
...
// This works (2D/2.5D view)
bus.emit(CameraEventTypes.setRotation, {
  id: instanceId,
  rotation: 90,
});

// Both are ignored in 3D view with Globe projection
```
...
- Pitch only works in 2.5D mode (not 2D or 3D)
...
Create smooth camera transitions using the built-in `transitionDuration` and `transitionEasing` properties. This approach leverages MapLibre's GPU-accelerated `easeTo` method for optimal performance:

```tsx
import { useEmit } from '@accelint/bus/react';
import { CameraEventTypes } from '@accelint/map-toolkit/camera';
import type { CameraSetCenterEvent } from '@accelint/map-toolkit/camera';

function FlyToLocation({ mapId }: { mapId: UniqueId }) {
  ...
  const flyToSanFrancisco = () => {
    setCenter({
      ...
      transitionEasing: 'ease-out', // 'linear' | 'ease-in' | 'ease-out' | 'ease-in-out'
    });
  };

  return <button onClick={flyToSanFrancisco}>Fly to San Francisco</button>;
}
```
...
- `ease-out` - Starts fast, decelerates (default)
- `ease-in-out` - Slow start and end, fast middle

**Alternative: Using animation libraries**

You can also create transitions with animation libraries like react-spring, but note that this approach has significant performance downsides:

> ⚠️ **Performance Warning**: Animation libraries like react-spring update camera position on every frame via React state and event bus emissions, running entirely on the CPU. This creates overhead from frequent re-renders and event processing. The built-in `transitionDuration`/`transitionEasing` approach is recommended as it uses MapLibre's GPU-accelerated `easeTo` method, which performs animations on the GPU without triggering React re-renders.
