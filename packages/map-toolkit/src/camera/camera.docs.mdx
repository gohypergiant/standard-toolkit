...
The hook also returns `pitchBy` and `rotateBy` actions for nudging the camera by a delta, without computing the absolute target yourself. These apply the same constraints as the events: `rotateBy` is ignored in 3D and normalizes to 0-360°, `pitchBy` only applies in 2.5D and clamps to 0-85°.
