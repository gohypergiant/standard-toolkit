...
Set the pitch (tilt) angle. Only works in 2.5D mode with Mercator projection.

```tsx
bus.emit(CameraEventTypes.setPitch, {
  id: instanceId,
  pitch: 45, // 0-85 degrees (typically 0-65 for best UX)
});
```

### pitchBy

Modify the pitch angle by a delta value. Only works in 2.5D mode. The result is automatically clamped to MapLibre's supported range of 0-85 degrees.

```tsx
bus.emit(CameraEventTypes.pitchBy, {
  id: instanceId,
  delta: 10, // increase pitch by 10 degrees
});

// Clamping example: if current pitch is 80°, delta of 20 results in 85° (clamped)
bus.emit(CameraEventTypes.pitchBy, {
  id: instanceId,
  delta: -100, // if current pitch is 30°, results in 0° (clamped at minimum)
});
```

### rotateBy

Modify the rotation angle by a delta value. Only works in 2D and 2.5D modes. The result is automatically normalized to the 0-360 degree range.

```tsx
bus.emit(CameraEventTypes.rotateBy, {
  id: instanceId,
  delta: 45, // rotate by 45 degrees
});

// Normalization examples:
// Current rotation 350° + delta 20° = 10° (wrapped around)
// Current rotation 5° + delta -15° = 350° (wrapped around)
// Current rotation 0° + delta 720° = 0° (full rotations normalized)
```
...
- `pitchBy`: Modify pitch by a delta value (clamped to 0-85°, only in 2.5D mode)
- `rotateBy`: Modify rotation by a delta value (normalized to 0-360°, only in 2D/2.5D modes)
...
The hook also returns `pitchBy` and `rotateBy` actions for nudging the camera by a delta, without computing the absolute target yourself. Useful when tying pitch and rotation mutations to mouse movement. These apply the same constraints as the events: `rotateBy` is ignored in 3D and normalizes to 0-360°, `pitchBy` only applies in 2.5D and clamps to 0-85°.

```tsx
const { pitchBy, rotateBy } = useMapCamera(MAP_ID);

// Tilt up 10° (clamped at 85°)
const handleTiltUp = () => pitchBy(10);

// Rotate 15° clockwise (wraps past 360°)
const handleRotateRight = () => rotateBy(15);
...
Pitch can only be set in 2.5D mode. Rotation can only be set in 2D and 2.5D modes. Both are ignored in 3D mode:
