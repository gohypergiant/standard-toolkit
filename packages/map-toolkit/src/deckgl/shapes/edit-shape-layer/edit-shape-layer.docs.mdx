...
- **Enter to confirm**: Press Enter key to confirm current changes
...
### shapes:edit-confirmed

Emitted when Enter key is pressed while editing.

```tsx
import { useOn } from '@accelint/bus/react';
import { EditShapeEvents } from '@accelint/map-toolkit/deckgl/shapes';

useOn(EditShapeEvents.confirmed, (event) => {
  if (event.payload.mapId !== MAP_ID) return;
  console.log('Edit confirmed for:', event.payload.shape.name);
});
```
...
| Option | Type | Description |
|--------|------|-------------|
| `mode` | `'bounding-transform' \| 'vertex-transform' \| 'circle-transform' \| 'translate'` | Override the automatic mode selection |
