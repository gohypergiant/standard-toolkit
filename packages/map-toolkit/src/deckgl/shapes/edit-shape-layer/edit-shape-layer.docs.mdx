...
| **LineString** | `modify-transform` | Drag vertices to reshape, OR scale/rotate/translate the entire shape |
| **Polygon** | `modify-transform` | Drag vertices to reshape, OR scale/rotate/translate the entire shape |
| **Rectangle** | `modify-transform` | Drag corner handles to resize, drag body to move, or rotate via handle. Hold Shift for uniform scaling. |
| **Circle** | `resize-circle` | Drag body to move, drag edge to resize from center |
| **Ellipse** | `ellipse-transform` | Drag to move, corner handles to scale freely, top handle to rotate. Hold Shift for uniform scaling. |
...
2. Click "Edit" to enter edit mode
3. **Resize**: Drag corners to resize (rectangle shape is maintained)
4. **Move**: Drag anywhere on the rectangle body to reposition
5. **Rotate**: Drag the top handle to rotate around the center
6. Hold **Shift** while dragging corners to constrain to a square
...
- **Shift** (hold while resizing rectangle): Constrain to square
- **Shift** (hold while scaling polygon/line/ellipse): Maintain aspect ratio (uniform scaling)
...
| Option | Type | Description |
|--------|------|-------------|
| `mode` | `'ellipse-transform' \| 'modify-transform' \| 'resize-circle' \| 'translate'` | Override the automatic mode selection |
