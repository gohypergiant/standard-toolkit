...
| **Ellipse** | `transform` | Drag to move, corner handles to scale uniformly, top handle to rotate |
...
4. **Scale**: Drag the corner handles to resize uniformly (maintains aspect ratio)
5. **Rotate**: Drag the top handle to rotate around the center
6. Click "Save" to confirm changes or "Cancel" (or press ESC) to discard
...
| Option | Type | Description |
|--------|------|-------------|
| `mode` | `'modify' \| 'resize-circle' \| 'transform'` | Override the automatic mode selection |
