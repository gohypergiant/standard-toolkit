...
`DrawShapeLayer` is a **React component** (not a deck.gl layer class), so it handles its own fiber registration internally. You do **not** need to import `draw-shape-layer/fiber`.
...
|------------|-----------------|
| **LineString** | Distance from last point (e.g., "12.5 km") |
| **Polygon** | Distance from last point (e.g., "12.5 km") |
| **Rectangle** | Dimensions and area (e.g., "5.2 km x 3.1 km\n16.12 km²") |
| **Circle** | Radius and area (e.g., "r: 12.5 km\n123.7 km²") |
| **Ellipse** | Distance (first click) then dimensions and area (second click) (e.g., "8.5 km x 5.2 km\n34.6 km²") |
...
| `unit` | `DistanceUnitSymbol` | `'km'` | Distance unit for tooltip measurements (supports `'km'`, `'m'`, `'NM'`, `'mi'`, `'ft'`) |
