...
- **Multiple geometry types**: Point, LineString, Polygon, Rectangle, and Circle
...
- **Custom styling**: Pass style defaults (fill/stroke colors) when initiating drawing
...
### With useDrawShapes Hook (Recommended)

The easiest way to use DrawShapeLayer is with the `useDrawShapes` hook, which manages drawing state and provides callbacks:
...
import { useDrawShapes } from '@accelint/map-toolkit/deckgl/shapes/draw-shape-layer/use-draw-shapes';
...
function MapWithDrawing() {
  ...
  const { draw, cancel, isDrawing, activeShapeType } = useDrawShapes(MAP_ID, {
    onCreate: (shape) => {
...
```tsx
const { draw } = useDrawShapes(MAP_ID, { onCreate: handleCreate });

// Draw with custom colors
draw(ShapeFeatureType.Polygon, {
  styleDefaults: {
    fillColor: [255, 100, 100, 180],   // RGBA: red fill
    strokeColor: [200, 0, 0, 255],      // RGBA: dark red stroke
    strokeWidth: 2,
    strokePattern: 'solid',
  },
});
...
| `options` | `UseDrawShapesOptions` | Optional callbacks |

#### Options

| Option | Type | Description |
|--------|------|-------------|
| `onCreate` | `(shape: DisplayShape) => void` | Called when shape is successfully created |
...
| Option | Type | Description |
|--------|------|-------------|
| `styleDefaults` | `Partial<StyleProperties>` | Default colors/stroke for the shape |
...
| `canceled` | `'shapes:canceled'` | Drawing was canceled |
...
|---------|-------|-------------|
| Line/Stroke | `#888a8f` | `--outline-interactive-hover` gray |
