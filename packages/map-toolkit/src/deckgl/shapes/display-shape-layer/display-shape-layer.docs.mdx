...
1. **Coffin corners** (Point shapes with icons): Bracket-like corners appear around selected Point icons with a blue fill. These are not currently customizable.
2. **Selection brightness overlay** (polygon shapes): A brightness overlay renders on top of the selected polygon using `MATERIAL_SETTINGS.HOVER_OR_SELECT` lighting. When the selected shape is also hovered, the hover layer stacks on top using `MATERIAL_SETTINGS.HOVER_AND_SELECT` for an even brighter combined effect.

### Hover Effects

When `pickable={true}`, shapes show hover feedback:

- **Polygon shapes**: Fill brightens via material-based lighting; stacks with any active selection treatment underneath
- **Non-polygon shapes (except Points with icons)**: Border/outline width increases by 2 pixels
...
- **Hover**: Curtain walls and indicator lines brighten to 1.25x their base color
- **Selection**: Curtain walls and indicator lines brighten to 1.25x their base color
- **Hover + Selection**: Curtain walls and indicator lines brighten to 1.5x their base color
