...
const getHoverColor = (feature: ShapeFeature): [number, number, number, number] => {
  return [255, 255, 0, 100]; // Yellow at ~39% opacity
};

// Custom select color - bright red overlay
const getSelectColor = (feature: ShapeFeature): [number, number, number, number] => {
...
**Note**: These overrides only affect the fill color of the hover/select overlay layers. Border brightness and width changes still apply as described above.
...
| `getHoverColor` | `(feature: ShapeFeature) => Rgba255Tuple` | `undefined` | Custom fill color function for hovered shapes. Overrides default brightness overlay. |
| `getSelectColor` | `(feature: ShapeFeature) => Rgba255Tuple` | `undefined` | Custom fill color function for selected shapes. Overrides default brightness overlay. |
