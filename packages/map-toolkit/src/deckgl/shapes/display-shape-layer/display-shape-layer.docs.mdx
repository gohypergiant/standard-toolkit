...
- **Interactive selection**: Click handling with visual brightness feedback; Points with icons show coffin corners
- **Hover effects**: Border/outline width increases on hover; fills and curtains brighten via material lighting; Points with icons show coffin corners
...
4. **Coffin corners** (Point shapes with icons): Bracket-like corners appear with a blue fill and black outline. The fill color defaults to `[57, 183, 250, 255]` (`#39B7FA`, fully opaque) and can be customized via `coffinCornerColor` (RGBA 0-255) when using the `CoffinCornersExtension` directly.
5. **Highlight** (optional, non-icon-Point shapes): A highlight layer underneath with:
...
- **Point shapes with icons**: Coffin corners appear with a white fill and black outline bracket
