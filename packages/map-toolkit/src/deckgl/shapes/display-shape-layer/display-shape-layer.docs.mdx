...
- **3D elevation rendering**: Extruded polygons, filled curtains for LineStrings, and vertical indicator lines for elevated points
...
|----------|-----------------|-------------|
| **Polygon / Rectangle / Circle / Ellipse** | `maxElevation` property on `StyledFeatureProperties` | Extruded upward from the ground to `maxElevation`. Z coordinates in geometry are ignored. |
| **LineString** | Per-vertex Z coordinates | Filled curtain walls drop from the line down to the ground |
| **Point** | Z coordinate | Vertical indicator line drops from the point down to the ground |
...
// Polygon at 22500m elevation via maxElevation (Z coordinates optional)
const polygon = {
  type: 'Feature',
  geometry: {
    type: 'Polygon',
