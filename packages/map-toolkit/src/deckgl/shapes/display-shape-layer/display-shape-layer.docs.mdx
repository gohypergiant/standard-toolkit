...
The layer automatically emits events via `@accelint/bus`. All events include an optional `id` field for multi-map scenarios.
...
| `mapId` | `UniqueId` | `undefined` | Map instance ID for event bus isolation in multi-map scenarios |
