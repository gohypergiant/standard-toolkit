...
Interactions never modify a shape's innate styling — hover and selection are always additive overlays.

When a shape is selected via `selectedShapeId`, the layer provides visual indicators:

1. **Line/outline brightness** (all shapes): The shape's border/outline brightens to 1.4× its base color.
2. **Fill brightness overlay** (polygon shapes): A brightness overlay renders using `MATERIAL_SETTINGS.HOVER_OR_SELECT` lighting. When the selected shape is also hovered, the hover layer stacks on top using `MATERIAL_SETTINGS.HOVER_AND_SELECT` for an even brighter combined effect.
...
- **All shapes**: Border/outline color brightens to 1.4× its base color
- **Polygon shapes**: Fill brightens via material-based lighting; stacks with any active selection treatment
...
By default, hover and selection states use a brightness overlay effect. You can override this with custom color functions:
...
```tsx
// Dynamic color based on feature properties
const getHoverFillColor = (feature: ShapeFeature): [number, number, number, number] => {
  const baseColor = feature.properties.styleProperties.fillColor;
  // Return a brighter version of the base color
  return [
    Math.min(255, baseColor[0] * 1.5),
    Math.min(255, baseColor[1] * 1.5),
    Math.min(255, baseColor[2] * 1.5),
    150, // Fixed alpha
  ];
};
```
...
- These overrides only apply to **polygon shapes** (Polygon, Rectangle, Circle, Ellipse, WagonWheel). Point and LineString shapes have no fill overlay, so the accessors are not invoked for them.
- Only the overlay fill is replaced. Border brightness and width changes from the default hover/select treatment still apply, and the selection outline color is still driven by [`highlightColor`](#api-reference) — set both if you want full control over selection appearance.
...
| `selectedShapeId` | `ShapeId` | `undefined` | Currently selected shape ID for showing the select layer |
...
| `getHoverFillColor` | `(feature: ShapeFeature) => Rgba255Tuple` | `undefined` | Custom fill color for the hover overlay (polygons only). Overrides the default brightness overlay. See [Custom Interaction Colors](#custom-interaction-colors). |
| `getSelectFillColor` | `(feature: ShapeFeature) => Rgba255Tuple` | `undefined` | Custom fill color for the selection overlay (polygons only). Overrides the default brightness overlay. See [Custom Interaction Colors](#custom-interaction-colors). |
