...
- **Hover effects**: Border/outline width increases on hover; fills and curtains brighten via material lighting; Points show coffin corners
...
2. **Fill brightness** (polygon shapes): The shape's fill renders at the un-dimmed base color brightened by 1.4× (1.7× when also hovered), instead of the standard `applyBaseOpacity`-reduced fill. The brightening can be replaced per-feature by [`getSelectFillColor`](#api-reference) and [`getHoverFillColor`](#api-reference) — see [Custom Interaction Colors](#custom-interaction-colors).
3. **Curtain brightness** (elevated LineString shapes): Curtain walls render at overlay opacity using a brightened version of the shape's line color.
...
- **Polygon shapes**: Fill renders at the un-dimmed base color brightened by 1.4× (1.7× when also selected); replaceable per-feature with [`getHoverFillColor`](#api-reference)
- **Elevated LineString shapes**: Curtain walls brighten at overlay opacity
...
- These accessors run for every feature in the layer, but only have a visible effect on shapes that render a fill — that's the **polygon family** (Polygon, Rectangle, Circle, Ellipse, WagonWheel) and unstyled Point shapes. LineStrings have no fill, and Points using icons have their color driven by the icon atlas, so the override is a no-op for those.
- Only the fill color is replaced. Border brightness and width changes from the default hover/select treatment still apply, and the selection outline color is still driven by [`highlightColor`](#api-reference) — set both if you want full control over selection appearance.
...
- **Hover or Selection**: Curtain walls render at overlay opacity using the shape's line color brightened to 1.4×
- **Hover + Selection**: Curtain walls render at overlay opacity using the shape's line color brightened to 1.7×

All polygons (2D and extruded) use a material-based hover effect (lighting change) for fill brightness. In 3D, this also compensates for deck.gl's inability to render outlines on extruded polygons.
...
| `getHoverFillColor` | `(feature: ShapeFeature) => Rgba255Tuple` | `undefined` | Custom fill color for the hovered shape. Replaces the default brightening on the main layer for that feature. See [Custom Interaction Colors](#custom-interaction-colors). |
| `getSelectFillColor` | `(feature: ShapeFeature) => Rgba255Tuple` | `undefined` | Custom fill color for the selected shape. Replaces the default brightening on the main layer for that feature. See [Custom Interaction Colors](#custom-interaction-colors). |
