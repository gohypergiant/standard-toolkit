...
Shapes with elevation coordinates render as 3D objects when `enableElevation` is true. Elevation is encoded as the third value in GeoJSON coordinate arrays, in meters above mean sea level (MSL).

### How It Works

Set `enableElevation={true}` to activate 3D rendering. When disabled (the default), shapes render flat regardless of their elevation coordinates.

Each geometry type gets a different 3D treatment:

| Geometry | 3D Behavior |
|----------|-------------|
| **Polygon / Rectangle / Circle / Ellipse** | Extruded upward from the ground to the shape's elevation |
| **LineString** | Filled curtain walls drop from the line down to the ground |
| **Point** | Vertical indicator line from the ground up to the point |

### Elevation Coordinates

Add elevation as the third coordinate value (meters MSL):
...
// Polygon at 22500m elevation (all vertices share the same elevation)
const polygon = {
  type: 'Feature',
  geometry: {
    type: 'Polygon',
