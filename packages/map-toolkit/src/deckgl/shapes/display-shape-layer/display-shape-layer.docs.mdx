...
- **Hover effects**: Line width increases on hover; Point shapes show coffin corners
      ...
      feature: {
        ...
        properties: {
          styleProperties: {
            ...
            strokeColor: [98, 166, 255, 255], // RGBA: blue at full opacity
            strokeWidth: 4,
            strokePattern: 'solid',
          },
        },
...
**Note**: By default, fill colors have a 60% opacity multiplier applied for the standard semi-transparent look. Set `applyBaseOpacity={false}` to render colors exactly as specified.
  ...
  feature: {
    ...
    properties: {
      styleProperties: {
        ...
        strokeColor: [98, 166, 255, 255], // RGBA
        strokeWidth: 4,
        strokePattern: 'solid',
        icon: {
          atlas: ATLAS_PNG,
...
1. **Dotted border**: The selected shape's stroke pattern changes to dotted (`[2, 4]` dash array)
2. **Coffin corners** (Point shapes): Bracket-like corners appear around selected Point icons with a blue fill (#39B7FA)
3. **Highlight** (optional, non-Point shapes): A highlight layer underneath with:
   - Increased line width (+5 pixels)
...
- **Non-Point shapes**: Line width increases by 2 pixels
- **Point shapes with icons**: Coffin corners appear with a white fill and black stroke
...
| `applyBaseOpacity` | `boolean` | `true` | Apply 60% opacity multiplier to fill colors for semi-transparent look |
