...
4. **Coffin corners** (Point shapes with icons): Bracket-like corners appear with a blue fill and black outline. The fill color defaults to `#39B7FA` and can be customized via `coffinCornerColor` when using the `CoffinCornersExtension` directly.
