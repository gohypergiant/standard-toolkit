...
**Note**: Colors are rendered exactly as specified. Use the `applyBaseOpacity` prop on the layer to apply a 60% opacity multiplier to fill colors for the standard semi-transparent look.
...
| `applyBaseOpacity` | `boolean` | `false` | Apply 60% opacity multiplier to fill colors for semi-transparent look |
