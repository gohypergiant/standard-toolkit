...
- [`@turf/area`](https://turfjs.org/) - Calculate circle area
- [`@turf/centroid`](https://turfjs.org/) - Calculate circle centroid
- [`@turf/circle`](https://turfjs.org/) - Create circle geometries
- [`@turf/distance`](https://turfjs.org/) - Calculate distances for circles
- [`@turf/helpers`](https://turfjs.org/) - Turf.js helper utilities
- [`@turf/length`](https://turfjs.org/) - Calculate circle perimeter length

**Installation:**

```bash
npm install @accelint/map-toolkit @accelint/bus @accelint/core @deck.gl/core @deck.gl/layers @deck.gl/extensions @deckgl-fiber-renderer/dom geojson react @turf/area @turf/centroid @turf/circle @turf/distance @turf/helpers @turf/length
# or
pnpm add @accelint/map-toolkit @accelint/bus @accelint/core @deck.gl/core @deck.gl/layers @deck.gl/extensions @deckgl-fiber-renderer/dom geojson react @turf/area @turf/centroid @turf/circle @turf/distance @turf/helpers @turf/length
```
...
The layer automatically emits events via `@accelint/bus`:
...
Emitted when clicking empty space (requires map click handler):
...
function MyComponent() {
  ...
  useOn(MapEvents.click, (event) => {
    ...
    if (selectedId && event.payload.info.index === -1) {
      emitDeselected(null);
    }
  });
...
- The layer uses deck.gl's `updateTriggers` to minimize re-renders
- Label positioning is calculated once per shape unless options change
- Icon atlases are shared across all Point geometries for efficiency
