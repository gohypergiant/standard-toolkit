...
function Toolbar({ mapId }: { mapId: UniqueId }) {
...
Unique identifier for this map instance. Used to isolate map mode state between multiple map instances (e.g., main map vs minimap). Must be of type `UniqueId`:
- For client components: Use React's `useId()` hook cast as `UniqueId` (recommended, prevents hydration issues)
- For module-level constants: Use `uuid()` from `@accelint/core`
