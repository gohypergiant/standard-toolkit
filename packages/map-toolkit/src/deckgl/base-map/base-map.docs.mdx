...
- **Tilt & Rotate**: Right-drag (or ctrl + left-drag) rotates the bearing and pitches the camera, driven through the camera store (left-drag still pans). A tilt gesture in flat `'2D'` auto-promotes the view to `'2.5D'` and tilts smoothly from the current pitch (0° in 2D) — it does not jump to the 60° default that an explicit view switch applies. Each new drag continues from the current angle, and pitch is bounded to 0-85°
...
| `defaultView` | `'2D' \| '2.5D' \| '3D'` | `'2D'` | Initial camera perspective. `'2D'` locks pitch at 0 (rotation allowed); `'2.5D'` enables mouse rotate + pitch (right-drag) up to 85°; `'3D'` is a fixed-orientation globe (pitch and rotation locked). |
...
`styleUrl` accepts any MapLibre style JSON URL. The package re-exports two Carto presets, LIGHT_BASE_MAP_STYLE and DARK_BASE_MAP_STYLE (default):
...
useOn(MapEvents.dragEnd, (payload) => {
...
- `MapEvents.dragStart`: `'map:dragStart'`
- `MapEvents.drag`: `'map:drag'`
- `MapEvents.dragEnd`: `'map:dragEnd'`
...
- **Controller**: Interactive with double-click zoom and roll disabled. MapLibre's own drag-rotate/pitch handlers stay off; rotation and pitch are driven through the camera store from a right-drag (or ctrl + left-drag) gesture, which auto-promotes flat `'2D'` to `'2.5D'`. `'3D'` keeps a fixed globe orientation
...
Shared by `MapEvents.dragStart` and `MapEvents.drag`:

Tilt and rotate gestures (right-drag, or Ctrl + left-drag) never emit drag events: BaseMap claims those for the camera, so bus consumers such as the measurement tool only ever see plain left-drags.

```typescript
type MapDragPayload = {
  id: UniqueId; // The map instance ID
  coordinate: [number, number]; // Unprojected [longitude, latitude] at the drag position
  shiftKey: boolean; // Whether Shift was held during the drag event
  ctrlKey: boolean; // Whether Ctrl was held during the drag event
  altKey: boolean; // Whether Alt was held during the drag event
};
```

`map:dragStart` and `map:drag` fire after the corresponding `onDragStart` / `onDrag` prop, and only when the pointer position unprojects to finite map coordinates (drags that start or move off the globe are skipped). The modifier flags let subscribers decide whether to claim a gesture — the [Measurement Layer](?path=/docs/deckgl-measurement-layer--docs)'s `useMeasurement` hook is built on these events.

### `MapDragEndPayload`

Emitted by `MapEvents.dragEnd`:

```typescript
type MapDragEndPayload = {
  id: UniqueId; // The map instance ID
  coordinate: [number, number] | null; // Unprojected [longitude, latitude] at release, or null off the globe
  shiftKey: boolean; // Whether Shift was held during the drag event
  ctrlKey: boolean; // Whether Ctrl was held during the drag event
  altKey: boolean; // Whether Alt was held during the drag event
};
...
Unlike the start and move events, `map:dragEnd` is always emitted when a drag gesture finishes, so subscribers that claimed the gesture can always release it. `coordinate` is `null` when the release position does not unproject to finite coordinates (for example, when the pointer is released off the globe in 3D view); check for `null` before using it.

The payload types and their `Payload` wrappers — `MapDragPayload`, `MapDragEndPayload`, `MapDragStartEvent`, `MapDragEvent`, and `MapDragEndEvent` — are exported from `@accelint/map-toolkit/deckgl` alongside the other `Map*` event types:

```typescript
import { MapEvents, type MapDragEndEvent } from '@accelint/map-toolkit/deckgl';
import { useOn } from '@accelint/bus/react';

useOn<MapDragEndEvent>(MapEvents.dragEnd, (event) => {
  if (event.payload.coordinate === null) {
    return; // released off the globe
  }

  const [longitude, latitude] = event.payload.coordinate;
});
```
