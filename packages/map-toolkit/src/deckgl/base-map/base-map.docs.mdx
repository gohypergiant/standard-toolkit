...
- **Tilt & Rotate**: In the `'2.5D'` view, right-drag rotates the bearing and pitches the camera (left-drag still pans); dampened from MapLibre's default sensitivity for finer control
...
- **Controller**: Interactive with double-click zoom and roll disabled. Mouse drag-rotate and pitch (right-drag) are enabled only in the `'2.5D'` view; `'2D'` stays flat and `'3D'` keeps a fixed globe orientation
