...
Initial camera view mode. Defaults to `'2D'`.

#### `initialViewState?: { zoom?: number; latitude?: number; longitude?: number }`

Initial map position. If not provided, falls back to the default view state from `@accelint/map-toolkit` constants.

#### `onViewStateChange?: (params: ViewStateChangeParameters) => void`

Callback fired when the map viewport changes (pan, zoom, rotate). Receives Deck.gl `ViewStateChangeParameters`.

#### `pickingRadius?: number`

Pixel radius for object picking. Creates a detection area around the pointer for pickable objects. Defaults to `PICKING_RADIUS` (5 pixels).
...
type MapClickPayload = {
  info: SerializablePickingInfo; // Modified PickingInfo for serialization
