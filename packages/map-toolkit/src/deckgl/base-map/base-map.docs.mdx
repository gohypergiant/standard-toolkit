...
- `MapEvents.centerOn`: `'map:centerOn'`
...
import { useEmit } from '@accelint/bus/react';
import { MapEvents } from '@accelint/map-toolkit/deckgl';
import type { MapCenterOnEvent } from '@accelint/map-toolkit/deckgl';

function FeatureList({ features }) {
  const emitCenterOn = useEmit<MapCenterOnEvent>(MapEvents.centerOn);
  ...
  return (
    <ul>
      {features.map((f) => (
        <li key={f.id} onClick={() => handleFeatureClick(f)}>
          {f.name}
        </li>
      ))}
    </ul>
...
Used by the `centerOn` event:

```typescript
type MapCenterOnPayload = {
  id: UniqueId;       // The map instance to center
  longitude: number;  // Longitude to center on
  latitude: number;   // Latitude to center on
