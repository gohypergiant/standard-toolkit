...
type MapClickPayload = {
  info: NonFuncPickingInfo; // PickingInfo with viewport, layer, and sourceLayer omitted for serialization
...
Note: Function properties, viewport, layer, and sourceLayer are omitted from event payloads to allow proper serialization through the event bus.
