...
export function MapView() {
  ...
  return (
    <BaseMap id={mapId} className="w-full h-full">
...
### Choosing Between `uuid()` and `useId()`

**For Client Components (Recommended):** Use React's `useId()` hook:
- ✅ Prevents hydration mismatches in SSR/Next.js applications
- ✅ Stable across server and client renders
- ✅ No import needed from `@accelint/core`

**For Module-Level Constants:** Use `uuid()` from `@accelint/core`:
- ✅ When you need a persistent ID across component instances
- ✅ For IDs that should be consistent across page reloads
- ⚠️ Only use at module level, never inside component render

**⚠️ Hydration Warning:**
If you use `uuid()` inside a React component (even with `useMemo` or `useState`), it will generate different IDs during server-side rendering and client hydration, causing React hydration errors. Always use `useId()` for component-level IDs.

```typescript
// ❌ NEVER - Causes hydration mismatch
function MapView() {
  const mapId = uuid(); // Different ID on server vs client!
  return <BaseMap id={mapId}>...</BaseMap>;
}

// ❌ STILL WRONG - useMemo doesn't prevent hydration issues
function MapView() {
  const mapId = useMemo(() => uuid(), []); // Still different on server vs client!
  return <BaseMap id={mapId}>...</BaseMap>;
}
...
// ✅ ALSO CORRECT - Module-level for shared IDs
...
UI components (toolbars, buttons, etc.) should be rendered as **siblings** to BaseMap, not children. When sharing IDs across components, create a context or pass the ID as props:
...
const MapIdContext = createContext<string>('');
...
#### `id: UniqueId | string` (required)

Unique identifier for this map instance. Used to isolate map mode state between multiple map instances (e.g., main map vs minimap). Accepts either:
- A string from React's `useId()` hook (recommended for client components)
- A `UniqueId` from `uuid()` in `@accelint/core` (for module-level constants)
...
// ✅ BEST - useId() for client components (prevents hydration issues)
// - Stable across server/client renders
// - No hydration mismatches in SSR/Next.js
...
// ✅ ALSO GOOD - Module-level constant for shared IDs
// - Stable across all renders and instances
// - Easy to import and share across components
// - Use when you need consistent IDs across reloads
const MAIN_MAP_ID = uuid();
...
function MapView() {
  const [mapId] = useState(() => uuid()); // Different ID on server vs client!
  return <BaseMap id={mapId}>...</BaseMap>;
}

// ❌ NEVER - Still causes hydration mismatch
function MapView() {
  const mapId = useMemo(() => uuid(), []); // Different ID on server vs client!
  return <BaseMap id={mapId}>...</BaseMap>;
}
```
...
MapLibre style URL for the base map tiles. Defaults to `DARK_BASE_MAP_STYLE` (Carto Dark Matter).
...
Whether to enable listening for map control events (pan/zoom enable/disable). When `true`, the map will respond to control events emitted via the event bus. Defaults to `true`.

Set to `false` if you don't need programmatic control over pan/zoom and want to skip the event listener setup:

```tsx
<BaseMap id={mapId} enableControlEvents={false}>
  <View id="main" controller />
</BaseMap>
```
...
// If using a shared module-level ID
import { uuid } from '@accelint/core';
const MAIN_MAP_ID = uuid();
