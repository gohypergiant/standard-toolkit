...
The recommended pattern uses `@accelint/bus` to synchronize viewport state between the map and saved viewport system. The key is using `CameraEventTypes.setCenter` to move the map when restoring a viewport:
...
// Create a stable map ID at module level
const MAP_ID = uuid();
...
const useSavedViewportHotkey = createSavedViewport({
  ...
  setCurrentViewport: (newState: MapViewState) => {
    ...
    // Emit camera event to move the map
...
export function MapView() {
  ...
  // Subscribe to viewport changes from BaseMap to track current position
...
**Important:** `MapEvents.viewport` is an *output* event emitted by `BaseMap` when the viewport changes. To *move* the map programmatically, you must emit `CameraEventTypes.setCenter` which the camera store listens to.
...
* **Saving**: Hold a number key (0-9) for the configured threshold (default 1 second) to save the current viewport
...
* **Camera Control**: Restored viewports emit `CameraEventTypes.setCenter` to move the map to the saved position
