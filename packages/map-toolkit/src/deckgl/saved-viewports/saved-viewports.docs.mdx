...
- **Hold to Save**: Hold a number key for 3 seconds to save the current viewport
...
const useSavedViewportHotkey = createSavedViewport({
  ...
  key: [
    { code: Keycode.KeyF, modifiers: ['ctrl'] }, // Ctrl+F
    { code: Keycode.KeyG, modifiers: ['ctrl'] }, // Ctrl+G
    { code: Keycode.KeyH, modifiers: ['ctrl'] }, // Ctrl+H
  ],
});
```
...
Milliseconds to hold a key before saving. Defaults to `3000` (3 seconds).
