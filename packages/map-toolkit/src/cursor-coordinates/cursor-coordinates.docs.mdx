...
|--------|-------------|---------|
| `dd` | Decimal Degrees | `37.774900° N / 122.419400° W` |
| `ddm` | Degrees Decimal Minutes | `37° 46.4940' N / 122° 25.1640' W` |
| `dms` | Degrees Minutes Seconds | `37° 46' 29.64″ N / 122° 25' 9.84″ W` |
| `mgrs` | Military Grid Reference System* | `10S EG 51130 80998` |
| `utm` | Universal Transverse Mercator* | `10N 551131 4180999` |

**Note:** UTM and MGRS formats are only valid from 80°S to 84°N inclusive. Coordinates outside this range will display `--- -- ---- ----`.
...
The hook's formatting is exposed as two plain functions from `@accelint/map-toolkit/cursor-coordinates`, for rendering a coordinate string outside the hook (a tooltip, a label, a log line) without re-deriving the grid conversion.

### `formatCoordinate(coord, format)`

Formats a `[longitude, latitude]` tuple into the requested system. Longitude is normalized to -180..180 first. Precision matches the hook: DD 6 decimal places, DDM 4 for minutes, DMS 2 for seconds.
...
formatCoordinate([-122.4194, 37.7749], 'utm');  // '10N 551131 4180999'
formatCoordinate([0, 85], 'mgrs');              // '--- -- ---- ----' (outside 80°S–84°N)
```

| Parameter | Type | Description |
|-----------|------|-------------|
| `coord` | `[number, number]` | `[longitude, latitude]` in decimal degrees |
| `format` | `CoordinateFormatTypes` | Target system: `'dd'`, `'ddm'`, `'dms'`, `'mgrs'`, or `'utm'` |

**Returns:** the formatted string, or the `--- -- ---- ----` placeholder for MGRS/UTM outside 80°S–84°N.

### `normalizeLongitude(lon)`

Wraps a longitude into -180..180, including multi-revolution values. `180` maps to `-180` (the range is half-open at the upper edge).

```tsx
import { normalizeLongitude } from '@accelint/map-toolkit/cursor-coordinates';

normalizeLongitude(190);  // -170
normalizeLongitude(-540); // -180
```

## Coordinate Format Limitations

### Polar Regions

UTM and MGRS coordinate systems have geographic limitations:

- **Valid Range**: 80°S to 84°N (inclusive at both edges)
- **Behavior at Poles**: When the cursor hovers over coordinates outside this range (e.g., Antarctica or Arctic regions), UTM and MGRS formats will display `--- -- ---- ----` (the default placeholder)
- **Other Formats**: DD (Decimal Degrees), DDM (Degrees Decimal Minutes), and DMS (Degrees Minutes Seconds) work correctly at all latitudes, including polar regions

**Example:**
```tsx
function PolarCoordinateDisplay() {
  const { formattedCoord, currentFormat, rawCoord } = useCursorCoordinates(MAP_ID);

  // At latitude -85° (Antarctica):
  // - DD format: "85.000000° S / 0.000000° E" ✓
  // - UTM format: "--- -- ---- ----" (outside valid range)
  // - rawCoord: { latitude: -85, longitude: 0 } ✓ (always available)

  return <div>{formattedCoord}</div>;
}
```

**Recommendation**: For applications covering polar regions, consider defaulting to DD, DDM, or DMS formats, or implement format switching logic based on latitude.
