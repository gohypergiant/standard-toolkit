...
- **Raw Coordinate Access**: Direct access to longitude, latitude, and tuple values for custom processing
...
function RawCoordinateDisplay() {
  ...
  return (
    <div>
      ...
      <div>Tuple: [{rawCoord.tuple[0]}, {rawCoord.tuple[1]}]</div>
...
```tsx
{
  ...
  tuple: [number, number];     // [longitude, latitude] tuple
} | null
```
