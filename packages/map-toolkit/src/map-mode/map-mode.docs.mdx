...
While the examples above use React, the core map-mode state management works with any framework or vanilla JavaScript. The module-level functions implement the [**subscribe/notify pattern**](https://react.dev/reference/react/useSyncExternalStore#subscribing-to-an-external-store) (also known as the Observer pattern) and can be used directly:
...
// Get the subscription function for this instance
const subscribe = getOrCreateSubscription(id);
const getSnapshot = getOrCreateSnapshot(id);
const requestModeChange = getOrCreateRequestModeChange(id);
...
requestModeChange('drawing', 'my-component-id');
