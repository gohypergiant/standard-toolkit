...
const MAIN_MAP_ID = uuid();
// `'default'` is always included in `mode`'s type — no need to list it here
type MapModes = 'drawing' | 'measuring';
type MapModeOwners = 'drawing-toolbar-id' | 'details-panel-id';

function DrawingToolbar() {
  // Use the shared id to access the map mode state
  const { mode, requestModeChange } = useMapMode<MapModes, MapModeOwners>(MAIN_MAP_ID);
...
While the examples above use React, the core map-mode state management works with any framework or vanilla JavaScript. You can use the store directly or interact via the event bus.

### Using the Store Directly

```typescript
import { modeStore, getMode, clearMapModeState } from '@accelint/map-toolkit/map-mode';
import { uuid } from '@accelint/core';
...
// Get current mode
const currentMode = getMode(id);
console.log('Current mode:', currentMode);

// Access the store for more control
const state = modeStore.get(id);
...
The two optional type parameters let you narrow `mode` and the arguments to `requestModeChange` at the call site. Both default to `string`, so existing untyped callers keep working unchanged.

#### Type Parameters

##### `TMode extends string` (default `string`)

String union of mode names the caller intends to use. `'default'` is always added to the returned `mode` type, so you don't list it yourself. The narrowing is advisory — the underlying store accepts any string, so values arriving from other bus participants aren't runtime-validated against this union.

##### `TOwner extends string` (default `string`)

String union of owner ids the caller intends to use when calling `requestModeChange`. Same advisory caveat as `TMode`.
...
```tsx
{
  mode: TMode | 'default';                     // Current mode
  requestModeChange: (                         // Request a mode change
    desiredMode: TMode | 'default',
    requestOwner: TOwner,
  ) => void;
}
```

When called without explicit type parameters, both `TMode` and `TOwner` default to `string`, matching the pre-generics signature.
...
type Owner = 'toolbar' | 'details-panel';

function App() {
  const mapId = uuid();

  function MapToolbar() {
    // Pass mapId since this is a sibling to BaseMap. Generics narrow the
    // accepted modes and owners — wrong strings fail at the type level.
...
**Behavior**: Map mode state is created automatically on first subscription when `useMapMode` is called.

**Key Points**:
- No manual store creation needed - state is lazily initialized
- Multiple components can use `useMapMode` with the same `id` safely
- Cleanup happens automatically when all subscribers unmount
- `clearMapModeState(id)` can be used for manual cleanup in advanced scenarios
...
**Impact**:
- Map mode state is properly cleaned up (no memory leaks)
