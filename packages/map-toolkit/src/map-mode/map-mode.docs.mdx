...
##### `Mode extends string` (default `string`)
...
##### `Owner extends string` (default `string`)

String union of owner ids the caller intends to use when calling `requestModeChange`. Same advisory caveat as `Mode`.
...
```tsx
{
  mode: Mode | 'default';                     // Current mode
  requestModeChange: (                         // Request a mode change
    desiredMode: Mode | 'default',
    requestOwner: Owner,
  ) => void;
}
```

When called without explicit type parameters, both `Mode` and `Owner` default to `string`, matching the pre-generics signature.
