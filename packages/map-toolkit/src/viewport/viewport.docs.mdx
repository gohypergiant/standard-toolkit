...
Available units (from `@accelint/constants/units`):
- `'km'` - Kilometers
- `'m'` - Meters
...
function MapInfo() {
  const { bounds, latitude, longitude, zoom } = useMapViewport(MAP_ID);
...
function BoundsMonitor({ mapId }: { mapId: UniqueId }) {
...
function ZoomLevel() {
  const { zoom } = useMapViewport(MAP_ID);
  return <div>Zoom: {zoom.toFixed(1)}</div>;
}

function CenterCoordinates() {
  const { latitude, longitude } = useMapViewport(MAP_ID);
...
```tsx
{
  ...
  bounds?: Bounds;           // [minLon, minLat, maxLon, maxLat], undefined before first event
...
**Initial State:** `bounds` is `undefined`, numeric coordinate values are `NaN`, and pixel dimensions are `0` before the first viewport event fires.
...
```tsx
// Displays: "-- x -- NM" when:
// - Bounds is undefined (before first viewport event)
...
```tsx
const { latitude, longitude, zoom, width, height } = useMapViewport(mapId);
...
// With useMapViewport for dynamic updates
function CustomViewportDisplay({ mapId }: { mapId: UniqueId }) {
...
```tsx
{
  ...
  width: number;                 // Viewport width in pixels
  height: number;                // Viewport height in pixels
  unit?: DistanceUnitSymbol;     // 'km' | 'm' | 'NM' | 'mi' | 'ft' (default: 'NM')
...
import type {
  Bounds,
  MapViewportPayload,
} from '@accelint/map-toolkit/deckgl/base-map';
import type {
  DistanceUnit,
  DistanceUnitSymbol,
...
// Display symbol for distance units (from @accelint/constants/units)
type DistanceUnitSymbol = 'km' | 'm' | 'NM' | 'mi' | 'ft';

// Full Turf.js unit names (for use with Turf functions)
type DistanceUnit = 'kilometers' | 'meters' | 'nauticalmiles' | 'miles' | 'feet';

// Viewport payload
type MapViewportPayload = {
  id: UniqueId;
  bounds?: Bounds; // undefined before first viewport event
  latitude: number;
  longitude: number;
  zoom: number;
  width: number;   // Viewport width in pixels
...
Distance unit symbols and types are defined in `@accelint/constants/units`:
...
// DISTANCE_UNIT_SYMBOLS maps Turf.js unit names to SI-compliant symbols:
// { kilometers: 'km', meters: 'm', nauticalmiles: 'NM', miles: 'mi', feet: 'ft' }
```

Reverse lookup (symbol → unit name) is available from `@accelint/constants/units`:

```tsx
import { DISTANCE_UNIT_BY_SYMBOL } from '@accelint/constants/units';

// Convert symbol to Turf.js unit name
DISTANCE_UNIT_BY_SYMBOL['NM']; // 'nauticalmiles'
DISTANCE_UNIT_BY_SYMBOL['km']; // 'kilometers'
```

Map-toolkit-specific helpers are in `@accelint/map-toolkit/shared/units`:

```tsx
import { DEFAULT_DISTANCE_UNITS } from '@accelint/map-toolkit/shared/units';

// Default units for measurements
DEFAULT_DISTANCE_UNITS; // 'kilometers'
```
...
### useMapViewport returns all NaN values
...
```tsx
const { latitude, width } = useMapViewport(mapId);
...
**Causes:**
1. Wrong `mapId` (doesn't match `BaseMap` id)
2. `BaseMap` not mounted

**Solutions:**
1. Verify `mapId` matches `BaseMap` `id` prop
