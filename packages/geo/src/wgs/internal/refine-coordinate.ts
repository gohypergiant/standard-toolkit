...
export function refineCoordinate(partial: RawCoordinate): RefinedCoordinate {
  ...
  const sign = COMPASS_SIGN[compass as keyof typeof COMPASS_SIGN] ?? 1;

  const raw = degrees + minutes / 60 + seconds / 3600;
  const value = compass ? Math.abs(raw) * sign : raw;

  const errors = [
    minutes < 60 ? '' : `Minutes value too high: "${minutes}"`,
    minutes < 0 ? `Minutes value too low: "${minutes}"` : '',
    seconds < 60 ? '' : `Seconds value too high: "${seconds}"`,
    seconds < 0 ? `Minutes value too low: "${seconds}"` : '',

    compass.toUpperCase() === 'N' && degrees < 0
      ? 'Conflicting indicators "negative degrees" and compass direction "North"'
      : '',
    compass.toUpperCase() === 'E' && degrees < 0
