...
export function buildCoordinate(
  ...
  if (Math.abs(coord.lat ?? 0) > 90) {
    return [
      new ParseError(`Latitude value out of range: "${coord.lat}"`),
      null,
    ];
  }

  if (Math.abs(coord.lon ?? 0) > 180) {
    return [
      new ParseError(`Longitude value out of range: "${coord.lon}"`),
      null,
    ];
  }

  return [null, coord as unknown as CoordinateWGS];
}
