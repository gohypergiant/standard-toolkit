...
This release removes support for the [Tailwind Variants](https://www.tailwind-variants.org/) library and instead uses [CSS Modules](https://github.com/css-modules/css-modules) as the primary css solution. Because the Design Foundation library will no longer export tailwind variant and tailwind merge utilities it will no longer be an officially supported approach.

See below for details on how to migrate your application and why we chose to refactor.
...
* **@layer** -- assigns the classes in that group to a layer. In an application-specific component, that would most likely be `l3`.
...
* **Pseudo-selectors** - although pseudo-selectors can be written with the `@variant` directive, there is a preference to use the direct CSS selectors pattern, with the ampersand colon syntax.
...
### 4. Edit your root global.css file _(only if necessary)_
...
The example script can be found here and modified for your code base - `[design-toolkit/components/scripts/token-migration.mjs](https://github.com/gohypergiant/standard-toolkit/blob/7ab50ade20f673c500d40abab1a3081e8796f35b/design-toolkit/components/scripts/token-migration.mjs)`.
