...
We are now using [tsdown](https://tsdown.dev/) instead of [tsup](https://tsup.egoist.dev/) for our build tool, which significantly improves build time.

# Breaking change

We are leveraging the [auto export](https://tsdown.dev/options/package-exports#auto-generating-package-exports) feature of tsdown.
By using auto export, all exports for the packages are added explicitly to `"exports"` in our package.json.
...
you'll notice that this export is pointing to an index.js file. We previously were using wildcards for exports in package.json, and the way packages are resolved in node necessitated explicit `/index` suffixes when importing from directory indexes.

from the [node docs on module resolution](https://nodejs.org/api/packages.html#module-resolution-and-loading):
> When a module is requested via static import statements (only available in ES Modules) or import() expressions (available in both CommonJS and ES Modules)... directory indexes (e.g. './startup/index.js') must be fully specified.

With the explicit exports we get from tsdown, the `/index` suffix has been removed, and the imports will break if you still use it.

## Migration steps

### 1. Fix any imports from design-toolkit or design-foundation that had the `/index` suffix

Remove the index suffix anywhere that it was used
