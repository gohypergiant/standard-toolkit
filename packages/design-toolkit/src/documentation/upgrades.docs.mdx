...
Add the following to your application's top-level component (e.g., `App.tsx`) to ensure the `@accelint/design-foundation/styles` CSS is available/loaded in your application:

```ts
import '@accelint/design-foundation/styles';
```
#### Importing within Storybook
Storybook's standard configuration folder is `.storybook`. The appropriate place to perform global imports (such as CSS imports) is inside of `.storybook/preview.ts`. [Details](https://storybook.js.org/docs/configure#configure-story-rendering)

Add the import statement above to `.storybook/preview.ts` to apply the `@accelint/design-foundation/styles` CSS to all stories.
...
At the top-level of your `src` directory, create `index.module.css` and add a `@reference` statement:
...
All CSS modules (files) in your application should then reference this top-level `index.module.css` file:

```css
/*
makes all classnames available for reference. this
is pointing to the top-level index.module.css.
*/
