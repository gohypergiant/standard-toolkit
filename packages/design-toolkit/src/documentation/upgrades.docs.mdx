...
* [Upgrade to v10](#v10)
* [Upgrade to v9.11](#v9_11)
* [Upgrade to v9.1](#v9_1)
* [Upgrade to v9](#v9)
* [Upgrade to v8](#v8)
* [Upgrade to v7](#v7)
* [Upgrade to v6](#v6)
...
## Breaking Change: TanStack Table v9

The `Table` component now runs on [TanStack Table v9](https://tanstack.com/table/latest/docs/framework/react/guide/migrating). The link covers the full v8 → v9 API migration; the TLDR for Design Toolkit consumers is:

### 1. Update the peer dependency

```json
"@tanstack/react-table": "^9.0.0",
```

### 2. Build columns with `createTableColumnHelper`

Column definitions are now typed against the Table's feature set. Replace TanStack's `createColumnHelper<TData>()` with the Design Toolkit's pre-bound helper:
...
If you reference TanStack types directly, pass the exported `TableFeatures` type as the first generic — e.g. `ColumnDef<TableFeatures, TData, TValue>`, `CellContext<TableFeatures, TData, TValue>`.

### 3. Update selection state types

TanStack's `RowSelectionState` is now `Record<string, true>`; update any `Record<string, boolean>` selection state accordingly.

### 4. Keep column defs referentially stable

TanStack v9 renders function `cell`/`header` renderers as React components, so a column definition recreated on each render remounts its cells on each render (resetting open menus, focus, and other cell state). Define columns at module scope or memoize them with stable dependencies.

## Improvements

* The Table now reflects `data` prop changes without a remount (e.g. polling or refetching) — the `key` remount workaround is no longer necessary.
* Manual row ordering is a TanStack custom feature (`rowOrderingFeature`, exported): `table.setRowOrdering`, `row.moveUp()` / `row.moveDown()`, and `row.getCanMoveUp()` / `row.getCanMoveDown()`. Moves skip pinned rows, and open row menus survive live data updates.

## Breaking Change: FloatingCard without dockview

`FloatingCard` no longer depends on [dockview](https://dockview.dev/). Dragging, resizing, and stacking order are handled by the component itself.

Every `FloatingCard` and `FloatingCardProvider` prop keeps its behavior, so most applications only need step 1.

### 1. Remove the peer dependency

`dockview-react` is no longer a peer dependency. If you installed it only for `FloatingCard`, drop it:
...
### 2. Replace `useFloatingCard().api`

The hook no longer returns `api`, which exposed the underlying `DockviewApi`. There is no equivalent, because there is no dockview instance. Use the hook's own methods instead:

```typescript
// Before
const { api } = useFloatingCard();
api?.getPanel(cardId)?.api.close();
...
### 3. Drop `addRef` and `removeRef`

The hook no longer returns `addRef` or `removeRef`. Registering a card is internal wiring between `FloatingCard` and its provider, so it moved off the value applications consume.

`useFloatingCard()` still returns `cards`, `closeCard`, `togglePinCard`, `isPinned`, and `subscribeToPinState`, all unchanged.

### 4. Label your custom header actions

Header action buttons render an icon with no visible text. Give each one a `label` so screen readers announce something other than "button":

```typescript
// Before
headerActions={[{ icon: <SettingsIcon />, onClick: openSettings }]}

// After
headerActions={[{ icon: <SettingsIcon />, label: 'Settings', onClick: openSettings }]}
```

## Improvements

* Cards can be resized from any edge or corner. Pinning a card continues to freeze dragging and now also freezes resizing.
* `FloatingCardProvider` accepts a `bounds` prop of `'provider'` (default) or `'viewport'`, controlling the region cards are confined to while dragging and resizing.

<hr />
<a id="v9_11"></a>
# Upgrade to v9.11

This upgrades the version of react-aria-components to [1.17.0](https://react-aria.adobe.com/releases/v1-17-0). In this version, they have [simplified their dependencies](https://react-aria.adobe.com/releases/v1-17-0#dependency-consolidation) which means that we can, too.

### Update Peer Dependencies
In order to take advantage of this in your own application, when upgrading you should remove any `@react-aria/*` and `@react-stately/*` dependencies and consolidate imports to just `react-aria` and `react-stately`. Sub-path imports will help keep your bundle size manageable. These are the peer dependency versions.
```
"react-aria": "~3.48.0",
...
### Simplify Overrides
You should be able to remove all or most of your dependency overrides if you previously applied them. Depending on the specifics of the application, you might still have overrides for supporting libraries. For example, an `@internationalized/date` override might still be required:
...
### Using sub-path imports

react-aria-components 1.17.0 introduces sub-path imports. Instead of importing everything from the main entry point, you can import from a component-specific sub-path. The main entry point still works — this is an optional optimization that gives you smaller bundles without relying on tree-shaking and faster builds and test runs.
...
We have documented the problem in [this PR](https://github.com/gohypergiant/standard-toolkit/pull/703) but broadly speaking an issue can occur when multiple versions of React Aria libraries are insalled within a project. The only guaranteed way of rectifying this issue is by specifying the version in your `package.json` `overrides` section. When upgrading to v9.1 of `@accelint/design-toolkit` ensure the following is added to your root `package.json`:
...
Note that if you are using `pnpm` you need to wrap `overrides` e.g.
...
## Breaking Change: /styles export removed from design-Toolkit
...
Remove the design-toolkit styles import from your global css. You should also remove the @source (though leaving it in doesn't technically break anything)
