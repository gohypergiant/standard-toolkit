...
[Go directly to migration steps for v8](#migration).
...
Therefore, any application using this library must configure CSS modules to leave named group classes un-hashed. **Failing to do so will break group-based state styling throughout the design system.** We now provide two custom functions `generateScopedClassName` and `getLocalIdent` which will prevent unwanted hashing when applied correctly. This ensures named group classes remain stable while all other classes continue to benefit from scoped CSS Module behavior.

**Webpack / Next.js**
Use the exported `getLocalIdent` function and apply it to every CSS loader rule. Because Webpack configurations vary—especially in Next.js—we do not ship automatic rule-discovery. Consumers must insert `getLocalIdent` into their own Webpack loader definitions, using our examples as a reference.
...
<a id="css-modules"></a>
### 3. Migrate to CSS modules and clsx
...
```css
/* makes all classnames available for reference */
...
* **@reference** - the use of the `@reference` directive at the top of your css module is required to avoid duplication of applied classes. [Details](https://tailwindcss.com/docs/functions-and-directives#reference-directive)

* **@layer** -- assigns the classes in that group to a layer. In an application-specific component, that would most likely be `l3`. [Detaiils](https://developer.mozilla.org/en-US/docs/Web/CSS/Reference/At-rules/@layer)
...
Although CSS Modules supports the composes keyword, it is not supported in this library. Even though local composes may appear to work, we recommend avoiding it for consistency. Instead, apply multiple classes directly in JSX (e.g., via clsx).

The main issue is with external composes (composes: `foo from './file.css'`). Tailwind processes styles before CSS Modules, but external composes pulls in the original, unprocessed source file. This bypasses Tailwind entirely, resulting in missing or incorrect styles. Because of how both tools operate, this cannot be reliably fixed. Instead, compose styles in JSX, not in CSS Modules. This ensures predictable Tailwind processing and consistent behavior across all components.

### 4. Common Mistakes & Troubleshooting

#### Mistake 1: Using external `composes` in CSS Modules
...
#### Mistake 2: Forgetting to configure named group hashing
...
**Solution**: Ensure you've configured `getLocalIdent` (Webpack/Next.js) or `generateScopedName` (Vite) as shown in migration steps 2.

**To verify**:
```tsx
// Inspect the DOM - you should see:
<button class="group/button _button_abc12">
  <svg class="group-hover/button:scale-110" />
</button>

// If you see this instead, hashing is broken:
<button class="_group__button_xyz99 _button_abc12">  {/* ❌ WRONG */}
```

#### Mistake 3: Using wrong layer for overrides
...
#### Mistake 4: Not importing `@reference` directive
**Problem**: Build output contains duplicate Tailwind utilities, bloating bundle size.

```css
/* ❌ WRONG - No @reference */
@layer components.l3 {
  .button {
    @apply px-4 py-2 rounded-md;  /* These get duplicated */
  }
}
```

**Solution**: Always include `@reference` at the top of CSS modules.
```css
/* ✅ CORRECT */
@reference '../../index.module.css';
...
@layer components.l3 {
  .button {
    @apply px-4 py-2 rounded-md;  /* Utilities referenced, not duplicated */
  }
}
```

#### Mistake 5: Incorrect variant prop naming
**Problem**: Badge or Chip components appear unstyled.

```tsx
{/* ❌ WRONG - variant prop no longer exists */}
<Badge variant="success">New</Badge>
<Chip variant="critical">Error</Chip>
```

**Solution**: Use `color` prop instead.
```tsx
{/* ✅ CORRECT */}
...
#### Troubleshooting: CSS modules not loading
**Symptoms**: Components render with no styles at all.

**Checklist**:
1. ✅ CSS modules enabled in build tool? (Next.js: built-in, Vite: check config)
2. ✅ Import path uses `.module.css` extension?
3. ✅ CSS file exists and is in correct location?
4. ✅ Build completed successfully without errors?
5. ✅ Check browser DevTools: are CSS files being loaded?
