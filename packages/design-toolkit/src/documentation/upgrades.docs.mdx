...
**Next.js 15+ with Turbopack (Recommended)**

If using Next.js 15+ with Turbopack (`next dev --turbo` or `next build --turbo`), no additional configuration is needed. Turbopack handles CSS modules correctly out of the box.
...
For applications still using webpack (including Next.js without Turbopack), use the exported `getLocalIdent` function and apply it to every CSS loader rule. Because webpack configurations vary—especially in Next.js—we do not ship automatic rule-discovery. Consumers must insert `getLocalIdent` into their own webpack loader definitions, using the example below as a reference.
