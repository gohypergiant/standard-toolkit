...
The `@react-aria/*`, `@react-stately/*`, `@react-types/*`, and `@internationalized/date` peer dependency ranges have been widened from tilde (`~`) to caret (`^`). This prevents version conflicts when `react-aria-components` resolves newer compatible versions of these sub-packages.

### Migration

If you added `overrides` (or `pnpm.overrides`) for React Aria sub-packages as recommended in the [v9.1 upgrade guide](#v9_1), you can now **remove them**. The widened peer dependency ranges allow these packages to resolve naturally without overrides.

You should keep the `react-aria-components` override at `~1.12.1` if you have it, as that package is still pinned to patch ranges.

After updating, run `pnpm dedupe` to consolidate any duplicate React Aria package versions in your lockfile. This ensures a single version of each sub-package is resolved across your project.
