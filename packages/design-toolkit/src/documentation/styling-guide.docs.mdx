...
[↑ Top](#styling-with-the-design-system)
Tailwind supports [variants](https://tailwindcss.com/docs/hover-focus-and-other-states) that allow for conditional application of styles, such as `:hover`, `:focus`, and `:active`. Additionally, in the Design Toolkit we have chosen to implement additional variants that are specific to [react aria components](https://react-aria.adobe.com/styling#plugin) which allow variants based on state. The full list of what is implemented can be [found here in variants.css](https://github.com/gohypergiant/standard-toolkit/blob/main/packages/design-foundation/src/variants/variants.css). You can combine variants for additional complexity or use them with the [`group` class](https://tailwindcss.com/docs/hover-focus-and-other-states#styling-based-on-the-descendants-of-a-group) provided by Tailwind.

```tsx
<div className="empty:hidden">{maybeContent}</div>

// using state for conditional utility classes
<Button className="pressed:bg-accent-primary-muted">
  {/* ... */}
</Button>

//can be used with grouped elements
<ToggleButton>
  <Icon>
...
//can be stacked
<div className="md:hover:bg-critical-bold ...">Important thing...</div>
```

Avoid using arbitrary variants, for example with a `data-` selector. Instead prefer the supported variants or conditional class rendering using the `clsx` library.
...
Tailwind supports [arbitrary values](https://tailwindcss.com/docs/adding-custom-styles#using-arbitrary-values) for one-off styles that don't exist in the theme. While tailwind provides bracketing to use arbitrary values, these should be avoided. In all cases, use these techniques sparingly and only when necessary:

```jsx
// ❌ Avoid using the bracket syntax
<div className="w-[325px]" />

// ✅ Ok to use arbitrary spacing values, if necessary
<div className="w-325" />
