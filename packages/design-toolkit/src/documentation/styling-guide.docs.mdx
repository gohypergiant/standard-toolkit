...
While the spacing tokens should cover the majority of cases and are preferred, it is possible to use arbitrary spacing values. The `w-<number>` and `size-<number>` utilities are driven by the `--spacing` theme variable, which is set to `1px` in the Design Toolkit. That means when you need to express arbitrary spacing values, you can do a direct translation.
...
While borders are not supported so that the style doesn't impact box model dimensions, you might have rare instances where you need to use a border. For example, if you need a line only at the top of a div. In this situation, use an outline semantic color variable directly to ensure consistency with the color system:

```jsx
<div className="border border-t-(--outline-static)" />
```
