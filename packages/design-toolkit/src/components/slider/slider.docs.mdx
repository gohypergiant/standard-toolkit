...
When you need discrete selection where users can only choose from predefined values, use the `snapToMarkers` prop. The slider will snap to the nearest marker value as you drag.
...
### Custom Intervals
Works with any marker configuration, even unevenly spaced values. The slider finds the nearest marker regardless of spacing.
...
* **`showValueLabels`** - Whether to display min/max value labels (default: true)
...
* **`onChangeEnd`** - Callback when value change is complete (e.g., on drag end)
...
* **`snapToMarkers`** - When true, the slider snaps to the nearest marker value (default: false). Requires markers to be defined. Works with any marker configuration including unevenly spaced values.
...
* **`classNames`** - Additional CSS classes for custom styling
  ...
  * `trackValue` - Applied to the slider track value (the filled portion)
  ...
  * `minValue` - Applied to the minimum value display
  * `maxValue` - Applied to the maximum value display
...
Marker positioning uses CSS custom properties via data attributes (`data-percent`), making styles overridable and avoiding inline styles.
