...
You should use one approach or the other, not both. The component will throw an error if you try to use both approaches simultaneously. The `items` object implies a dynamic collection, but the `expandedKeys` prop is trying to pass in controlled component data. In this example, you would remove the `expandedKeys` and put the `isExpanded` prop on the nodes in your `items` object that you want expanded.
