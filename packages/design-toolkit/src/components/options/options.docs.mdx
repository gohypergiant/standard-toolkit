...
- **OptionsSection** - Grouping container with optional header
...
- **`selectionMode`** - Selection behavior: `'none'` | `'single'` | `'multiple'` (default: `'single'`)
- **`selectedKeys`** - Set of selected item keys for controlled selection
...
### OptionsItem Props
- **`color`** - Color variant: `'info'` | `'serious'` | `'critical'` (default: `'info'`)
...
### OptionsItemContent Props
- **`children`** - Label and description components
- **`className`** - Custom CSS class name

### OptionsItemLabel Props
- **`children`** - Label text content
- **`className`** - Custom CSS class name

### OptionsItemDescription Props
...
// Serious color - for warnings or caution actions
<OptionsItem color="serious">...</OptionsItem>

// Critical color - for urgent or dangerous actions
<OptionsItem color="critical">...</OptionsItem>
```

### Selection Modes
Control how users can select items:
...
For controlled selection, use `selectedKeys` and `onSelectionChange`:
```tsx
