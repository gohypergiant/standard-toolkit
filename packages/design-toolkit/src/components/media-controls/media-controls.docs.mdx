...
* **`children`** - Child control components to render
* **`className`** - CSS class for styling
* **`classNames.container`** - Applied to the main container element
* **`isDisabled`** - Disable all child controls (default: `false`)

### PlayButton

* **`isDisabled`** - Disable this specific control (overrides context-level isDisabled)
...
* **`seekOffset`** - Seek amount in seconds (default: `10`)
* **`isDisabled`** - Disable this specific control (overrides context-level isDisabled)
...
* **`showLabel`** - Whether to show volume label (default: `false`)
* **`isDisabled`** - Disable this specific control (overrides context-level isDisabled)
...
* **`mode`** - Display mode: `'current'` (default), `'remaining'`, or `'duration'`
* **`placeholder`** - Text to display before media loads (default: `'hh:mm:ss'`)
* **`isDisabled`** - Disable this specific control (overrides context-level isDisabled)

### PlaybackRateButton

* **`rates`** - Array of available rates (default: `[1, 2, 3]`)
* **`isDisabled`** - Disable this specific control (overrides context-level isDisabled)
...
- **useResolvedDisabled** - Hook to resolve isDisabled state from prop or context
