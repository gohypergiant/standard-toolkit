...
## Readonly
Set `isReadOnly` on the component and supply the `value`. If `value` is not provided, it will render an empty space.
...
* **`isReadOnly`** - Whether the component is read-only
