...
```tsx
const ids = { drawer: uuid(), a: uuid(), b: uuid() };
...
| Prop | Type | Default | Description |
|------|------|---------|-------------|
| `for` | `SimpleEvents \| TargetedEvents \| ChainedEvents` | - | Action or view ID (see below) |

**`for` prop values:**
- **Simple events** (`'back' | 'clear' | 'close' | 'reset' | UniqueId`): Only work inside a Drawer context. Passing a `UniqueId` pushes that view onto its parent's stack.
- **Targeted events** (`'open:id' | 'close:id' | 'toggle:id' | 'reset:id'`): Control a specific drawer externally. `open` clears the stack before pushing the new view.
- **Chained events** (`Array`): Fire multiple events from a single trigger, e.g. `['reset', viewId]` or `` [`open:${tabOneId}`, `open:${tabCId}`] ``.

### DrawerPanel

Container for the drawer's scrollable content area. Accepts standard `<div>` props.

### DrawerContent

Wrapper for main content within a drawer panel. Accepts standard `<div>` props.

### DrawerFooter

Footer area pinned to the bottom of the drawer. Accepts standard `<footer>` props.

## Programmatic Control

The `useDrawerEmit` hook provides programmatic control over drawer state and navigation. It combines [ViewStack](?path=/docs/components-viewstack--playground) methods with drawer-specific actions.

### useDrawerEmit

```tsx
import { useDrawerEmit } from '@accelint/design-toolkit';
...
function MyComponent() {
  ...
  emit.push(viewId);      // Push a view onto the stack
  emit.pop(viewId);       // Pop the current view
  emit.back(drawerId);    // Navigate back one view
  emit.clear(drawerId);   // Clear the view stack
  emit.reset(drawerId);   // Reset to default view
}
```
...
|--------|-----------|-------------|
| `open` | `UniqueId` | Opens the drawer and navigates to the specified view |
| `toggle` | `UniqueId` | Toggles the drawer open/closed for the specified view |
| `close` | `UniqueId` | Closes the drawer |
| `push` | `UniqueId` | Pushes a view onto the navigation stack |
| `pop` | `UniqueId` | Pops the current view from the stack |
| `back` | `UniqueId` | Navigates back one view in the stack |
| `clear` | `UniqueId` | Clears all views from the stack |
| `reset` | `UniqueId` | Resets the stack to the default view |

All methods accept a `UniqueId` parameter identifying the drawer or view to control.
