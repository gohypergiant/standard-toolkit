...
- **[DrawerLayout](?path=/docs/components-drawerlayout--docs)** – Shared layout container (also used by Sidenav) for managing panel positioning with `extend` and `push` behavior
...
- **DrawerClose** – Heading element, closes drawer/view
...
- **`placement`** - Position of the drawer: `'top'`, `'bottom'`, `'left'`, `'right'` (default: `'left'`)
- **`size`** - Size of the drawer panel: `'small'`, `'medium'`, `'large'` (default: `'medium'`)
- **`defaultView`** - ID of the view to display when the drawer first opens
- **`onChange`** - Callback fired when the active view changes

## DrawerMenu Props

- **`position`** - Position of menu items within the menu bar: `'start'`, `'center'`, `'end'` (default: `'center'`)
...
Allows users to pass an ID for a given drawer/view, which will then emit a `DrawerClose` event via bus.
- **`for`** - Unique identifier for the tab to be closed.

## DrawerMenuItem

The DrawerMenuItem component represents individual navigation items within the drawer menu.

- **`for`** (required) - The unique identifier of the view this item should navigate to
- **`toggle`** - When `true`, clicking toggles the view open/closed. When `false`, clicking only opens. (default: `false`)
- **`textValue`** (required) - Text displayed in a tooltip when hovering over the menu item
