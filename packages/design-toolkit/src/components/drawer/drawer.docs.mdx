...
Allows uses to pass an ID for a given drawer/view, which will then emit a `DrawerClose` event via bus.
- **`id`** - Unique identifier for the tab to be closed. If not provided, bus will emit a `reset` event.
