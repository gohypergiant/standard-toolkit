...
- **`isClearable`** - Whether to show a clear button (default: `true`)
- **`isInvalid`** - Whether to show the input in an invalid state (default: `false`)

### Content
- **`prefix`** - Content rendered before the input (e.g., icons, currency symbols)
- **`suffix`** - Content rendered after the input (e.g., units, icons)
