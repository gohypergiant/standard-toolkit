...
export const Default: Story = {
  render: ({ children, ...args }) => (
    <div className='h-[400px] w-[300px]'>
      <ListView {...args} items={items} aria-label='Animals list'>
        {(item) => (
          <ListViewItem
            key={item.id}
            id={item.id}
...
const manyItems: CustomListViewItem[] = Array.from(
  { length: 5000 },
  (_, index) => ({
    id: index,
    name: `Item ${index}`,
...
export const Virtualized: Story = {
  render: ({ ...args }) => (
    <div className='h-[400px] w-[300px]'>
      <ListView {...args} aria-label='Virtualized items list' items={manyItems}>
        {(item) => (
          <ListViewItem key={item.id} id={item.id} textValue={item.name}>
            {item.prefixIcon && <Icon>{item.prefixIcon}</Icon>}
            <ListViewItemContent>
              <ListViewItemTitle>{item.name}</ListViewItemTitle>
              {item.description && (
