...
* **`noHotkeys`** - Disable all keyboard shortcuts (space=play, arrows=seek, m=mute). Useful when embedding in forms or apps with their own hotkey system.
* **`hotkeys`** - Custom hotkey configuration string. See [media-chrome hotkeys docs](https://www.media-chrome.org/docs/en/keyboard-shortcuts).
* **`noVolumePref`** - Don't persist volume level to localStorage across page loads.
* **`noMutedPref`** - Don't persist muted state to localStorage across page loads.
* **`lang`** - Language for media-chrome's built-in accessibility labels.
