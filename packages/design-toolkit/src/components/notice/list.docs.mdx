...
|-------|-------------|---------|
| `queue` | Add a notice to the queue | `{ id?, message, color?, primary?, secondary?, metadata?, timeout?, target? }` |
| `dequeue` | Remove notice(s) from the queue | `{ id?, color?, target?, metadata? }` |
| `actionPrimary` | Primary button was pressed | `{ id, metadata? }` |
| `actionSecondary` | Secondary button was pressed | `{ id, metadata? }` |
| `close` | Close button was pressed | `{ id, metadata? }` |
...
// Notice with action buttons and metadata
emit({
  message: "Layer failed to load",
  primary: { children: "Center On" },
  metadata: { layerId: "layer-123", geometry: featureCollection }
});
```

### Handling Action Events with Metadata

Action events include metadata so you can handle button clicks with full context:
...
function MapComponent() {
  ...
  const emit = useEmit(NoticeEventTypes.queue);

  // Emit a notice with metadata and action button
  const showLayerError = (layer) => {
    emit({
      message: `Layer "${layer.name}" failed to load`,
      color: "critical",
      primary: { children: "Center On" },
      metadata: { 
        layerId: layer.id,
  ...
  // Handle primary action clicks - metadata is passed in the event
  useOn(NoticeEventTypes.actionPrimary, (event) => {
  ...
  return <NoticeList id="map-notices" placement="top right" />;
}
```

**Note:** `NoticeList` uses event-driven architecture. You cannot pass `onPrimaryAction` directly as a prop - instead, use `useOn` to listen for action events on the bus.
