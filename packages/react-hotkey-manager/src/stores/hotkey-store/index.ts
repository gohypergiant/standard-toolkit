...
type HotkeyState = {
   ...
   */
  registeredKeyCominations: Map<KeyCombinationId, HotkeyId[]>;
  /**
   * The active key combinations
   */
  activeKeyCombintations: Map<KeyCombinationId, HotkeyId[]>;
  /**
...
export const hotkeyStore = createStore<HotkeyState & HotkeyActions>()(
  immer((set, get) => ({
    allHotkeys: new Map(),
    registeredKeyCominations: new Map(),
    activeKeyCombintations: new Map(),
      ...
      set((state) => {
        ...
        for (const key of config.key) {
          ...
          if (
            ...
            state.registeredKeyCominations.has(key.id)
          ) {
            const ids = state.registeredKeyCominations.get(key.id) as string[];
            console.warn(
          ...
          state.registeredKeyCominations.set(
            key.id,
            (state.registeredKeyCominations.get(key.id) ?? []).concat(
              config.id,
            ),
          );
        }
    ...
    activateHotkey: (id, activationSymbol) =>
      set((state) => {
        ...
        for (const key of config.key) {
          const activated = state.activeKeyCombintations.get(key.id) ?? [];

          if (activated.includes(id)) {
            continue;
          }

          state.activeKeyCombintations.set(key.id, [...activated, id]);
        }
    ...
    getHotkeysForKeyCombination: (keyCombination) => {
      ...
      const ids = state.activeKeyCombintations.get(keyCombination);
...
function removeHotkeyFromRegisteredKeyCombinations(
  ...
  for (const key of config.key) {
    const registered =
      state.registeredKeyCominations.get(key.id) ??
      /* istanbul ignore next -- @preserve */ [];
    const newRegistered = registered.filter((configId) => configId !== id);

    newRegistered.length === 0
      ? state.registeredKeyCominations.delete(key.id)
      : state.registeredKeyCominations.set(key.id, newRegistered);
  }
}
...
function removeHotkeyFromActiveKeyCombinations(
  ...
  for (const key of config.key) {
    const active =
      state.activeKeyCombintations.get(key.id) ??
      /* istanbul ignore next -- @preserve */ [];
    const newActive = active.filter((configId) => configId !== id);

    newActive.length === 0
      ? state.activeKeyCombintations.delete(key.id)
      : state.activeKeyCombintations.set(key.id, newActive);
  }
}
